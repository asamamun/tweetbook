		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col-lg-8">
					...
					$pagi = "SELECT COUNT(*) AS total FROM tweets WHERE privacy='1' and cid=".$cid." ";
					...
					$q = "SELECT tweets.*,categories.name  FROM `tweets` 
                    inner join categories on tweets.cid=categories.id
                    WHERE tweets.privacy = '1' and tweets.deleted is NULL and tweets.cid=".$cid." order by tweets.created desc LIMIT " . $recordStart . "," . PAGE_SIZE . "";
					 //echo $q;
...
';
						...
						$html .= ' </article>';
                        echo '<h2 class="font-weight-bold text-gray mb-3">All '.$catname.' Post</h2>';
						echo $html;
					}
					?>
					...
					<div class="pages text-center mt-4 mt-lg-0">
						...
						<a class="icon fas fa-chevron-left <?php echo ($defaultpage <= 1) ? "display" : "" ?>" href="<?php echo $_SERVER['PHP_SELF'] . '?page=' . $defaultpage - 1 ?>&cid=<?php echo $cid; ?>"></a>
						...
						for ($i = 1; $i <= $totalPage; $i++) {
							if (abs($defaultpage - $i) < 3) {
								...
								if ($defaultpage == $i) {
									echo '<a class="page active" href="' . $_SERVER['PHP_SELF'] . '?page=' . $i . '&cid='.$cid.'">' . $i . '</a>';
								} else {
									echo '<a class="page" href="' . $_SERVER['PHP_SELF'] . '?page=' . $i . '&cid='.$cid.'">' . $i . '</a>';
								}
							}
						}
						?>
						...
						<a class="icon fas fa-chevron-right <?php echo ($defaultpage == $totalPage) ? "display" : "" ?>" href="<?php
																																echo $_SERVER['PHP_SELF'] . '?page=' . $defaultpage + 1 ?>&cid=<?php echo $cid; ?>"></a>
					</div>
